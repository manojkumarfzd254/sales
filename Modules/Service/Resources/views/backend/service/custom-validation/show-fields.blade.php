...
<form action="{{route('backend.service.submitServiceFormData',['id'=>$$module_name_singular])}}" method="post" enctype="multipart/form-data">
    @csrf
@foreach ($fieldsData as $item)
    @if ($item['field_type'] == 'text' || $item['field_type'] == 'email' || $item['field_type'] == 'date' || $item['field_type'] == 'color' || $item['field_type'] == 'file')
        <div class="row mb-3">
