...
if (! function_exists('language_direction')) {
    ...
    // image upload code by rahul

     // function for upload image
     function uploadImage($input, $path = '', $storage = 'local')
    {
        if ($storage == 'local') {
    ...
    function history($id,$module_name,$action)
    {
        History::create([
            'perform_id' => $id,
    ...
    function recentHistory($id,$module_name,$action)
    {
...
if(!function_exists('generateQrcode'))
{
    ...
    {
        $qrcode = QrCode::size(400)->generate($data, public_path($path) );
        return $path;
    }
}

if(!function_exists('makeTreeforDepartments'))
{
    function makeTreeForDepartments($tree)
    {
        // dd($tree);
        foreach($tree as  $value){
            ...
            {
                echo '<li data-icon-cls="treeview-icon icon-folder" data-expanded="true"><img src="'.$value['icon'].'" style="width:20px;height:20px"> '.$value['name'];
                echo '<ul>';
                makeTreeForDepartments($value['children']);
                echo '</ul></li>';
            }
            else
            {
                echo '<li data-icon-cls="treeview-icon icon-txt"><img src="'.$value['icon'].'" style="width:20px;height:20px"> '.$value['name'].'</li>';
            }
...
if(!function_exists('checkFile'))
{
    ...
    {
        $extensions = ["pdf","xlsx","xls","csv","pptx","ppt","doc","docx"];
        $ext = strtolower(pathinfo($value, PATHINFO_EXTENSION)); // Using strtolower to overcome case sensitive
        if (in_array($ext, $extensions)) {
            return true;
        } else {
...
{
    function validate_email($email, $url = 'https://api.hunter.io/v2/email-verifier', $apiKey = 'aa05f7f3bf43960ca0db08e1d9604ba0c9846b87')
    {
        $params = array(
            'email' => $email,
            'api_key' => $apiKey
...
function callScripts($script)
{

    try {
        $process = Process::fromShellCommandline('php /var/www/html/salesdata/artisan ' . $script . ' > /dev/null 2>&1 &');
        $process->mustRun();
